...
const SocialLinks = () => {
  ...
  const otherLinks = socialLinks.filter(link => !link.isPrimary && !link.disabled);
  
  // Filter out disabled links and arrange the remaining ones
  const activeLinks = otherLinks.filter(link => link.name !== "Instagram" && link.name !== "TikTok");
  
  // Destructure only the available links
  const [youtube, github] = activeLinks;
